var DbUrlList,
    ...
    CrawlerUrl = require("supercrawler").Url,
...
DbUrlList.prototype._makeUrlRow = function (url) {
  ...
  let pathname = (new URL(url.getUrl())).pathname;

  let nextRetryDate = url.getErrorCode() === null 
    ? new Date((new Date()).getTime() - Math.random() * YEAR_MS / pathname.length) 
    : this._calcNextRetryDate(1);
...
DbUrlList.prototype.getNextUrl = function () {
  ...
  return this._getUrlTable().then(function (urlTable) {
    return urlTable.findOne({
      where: Sequelize.literal("nextRetryDate < 'now'"),
      order: ["nextRetryDate"]
